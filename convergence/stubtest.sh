#!/bin/bash
# Topology test: the reference document's structure ends in unpaired outer fingers,
# ours does not. Same stack, same mesh, same domain, one variable changed.
# Delta-length between the two lengths gives the stub contribution per column.
set -u
cd "$(dirname "$0")"
...
for lc in 0.120 0.080; do
  t=${lc/./p}
  run_one stub_l5p5_lc${t} $GDS/stub_l5p5_w7.json $lc
  run_one stub_l10_lc${t}  $GDS/stub_l10_w7.json  $lc
done
echo "STUBTEST DONE"
